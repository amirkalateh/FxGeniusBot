...
""",
    ...
    def save_message_to_db(self, message: Union[SystemMessage, HumanMessage, AIMessage]) -> None:
        ...
        try:
            ...
            # Extract timestamp from additional_kwargs if present
            ...
            session.add(db_msg)
            session.commit()
            print(f"✅ Saved message to DB: {msg_type} - {message.content}")
        except Exception as e:
            session.rollback()
...
4. Use the search_user_info tool TRAVILY to find specific user details 
...
class ConversationManager:
    ...
    def process_message(self, user_input: str):
        ...
        try:
            # Add user message to history before sending to agent
            user_message = HumanMessage(content=user_input, additional_kwargs={"timestamp": datetime.utcnow().isoformat()})
