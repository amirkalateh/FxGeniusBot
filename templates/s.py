...
class Message(Base):
    ...
    def to_message_object(
            self) -> Union[SystemMessage, HumanMessage, AIMessage]:
        """
...
class Fact(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    question = Column(Text, nullable=True, unique=True)
    answer = Column(Text, nullable=False)
    user_id = Column(String, index=True, nullable=True)
    score = Column(Integer,
                   default=0)  # Score to determine importance or relevance
    timestamp = Column(DateTime, default=datetime.utcnow, nullable=False)
    extra_metadata = Column(JSON,
                            nullable=True)  # Ensuring metadata can be stored
        ...
        try:
            ...
            if db_messages:
                ...
                system_message = SystemMessage(
                    content=
                    """You are a helpful AI assistant. Remember important information about users. 
...
TODAY IS 7 NOVEMBER 2024 (YOU SHOULD SEARCH WITH A PRIORITY ON THIS DAYS AGO FOR RAG THINGS LIKE WINNER OF US ELECTION!..).
ALSO REMEMBER THAT YOU ARE AN EFFICIENT SUPER INTELIGENT AGENT, NOT A JOURNALIST. ON A REAL TIME QUESTION WHEN YOU FIND AN ABSOLUTE ANSWER , TELL IT TO USER FIRMLY AND CONFIDENTLY. DONT MENTION THAT YOU CAN SEARCH AND LIKE THIS... 
ANSWER ABSOLUTE MUCH AS YOU CAN
""",
                    additional_kwargs={
    ...
    def save_message_to_db(
        ...
        """Log and save a single message to the database"""
        session = get_db_session()
        try:
            if isinstance(message, SystemMessage):
                msg_type = "system"
            ...
            db_msg = Message(
                ...
                timestamp=timestamp,
                extra_metadata=getattr(message, 'additional_kwargs', {}),
            )
            logger.info(f"Attempting to save message to DB: {db_msg}")
            session.add(db_msg)
            session.commit()
            logger.info(f"✅ Saved message to DB: {msg_type} - {message.content}")
        except Exception as e:
            session.rollback()
            logger.error(f"Error saving message to DB: {e}, Message: {message.content}")
        finally:
            session.close()
    ...
    def _process_ai_memory(self, message: AIMessage) -> None:
        ...
        # Keywords that might indicate factual information
        fact_triggers = [
            "fact",
            "it's stated that",
            "research shows",
        ...
        content_lower = message.content.lower() if isinstance(
            message.content, str) else ""

        # Check and save content to facts if fact-related triggers are found
        if any(trigger in content_lower for trigger in fact_triggers):
            fact_text = message.content  # This should be the actual content it needs to store
            reliability_score = 0.8  # Example static reliability score, adjust based on content

            memory_store.save_fact(fact_text, reliability_score, {
                "source": "from_AI_message",
                "raw_message": message.content
            })

        # Save as memory if memory-related triggers are found
    ...
    def get_messages(
            self) -> List[Union[SystemMessage, HumanMessage, AIMessage]]:
        """Get all messages from current thread with system message validation"""
        if not self.messages or not any(
                isinstance(m, SystemMessage) for m in self.messages):
            system_message = SystemMessage(
                content=
                """You are a helpful AI assistant. Remember important information about users. 
Use tools effectively to search for information and save key user details. 
...
ANSWER ABSOLUTE MUCH AS YOU CAN.""",
    ...
    def get_thread_summary(self) -> str:
        ...
        last_msg_time = last_msg.additional_kwargs.get(
            "timestamp",
...
class UserMemoryStore:
    ...
    def get_memories(self, user_id: str) -> List[MemoryEntry]:
        ...
        try:
            db_memories = (session.query(MemoryModelDB).filter_by(
                user_id=user_id).order_by(MemoryModelDB.timestamp).all())
            ...
            print(
                f"📥 Retrieved {len(memories)} memories from DB for user {user_id}"
            )
        except Exception as e:
        ...
        try:
            db_memories = (session.query(MemoryModelDB).filter_by(
                user_id=user_id).order_by(
                    MemoryModelDB.timestamp.desc()).limit(limit).all())
            for mem in db_memories:
                memories.append({
                    "content": mem.content,
                    "timestamp": mem.timestamp.isoformat(),
            ...
            print(
                f"📥 Retrieved {len(memories)} recent memories from DB for user {user_id}"
            )
        except Exception as e:
    ...
    def search_memories(self, user_id: str, query: str) -> List[MemoryEntry]:
        """
        ...
        try:
            db_memories = (session.query(MemoryModelDB).filter(
                MemoryModelDB.user_id == user_id).filter(
            ...
            print(
                f"🔍 Found {len(results)} memories matching '{query}' for user {user_id}"
            )
        except Exception as e:
    ...
    def save_fact(
        ...
        """
        Save a fact with a source and reliability score
        """
        session = get_db_session()
        try:
            existing_fact = session.query(Fact).filter_by(answer=fact).first()
            if not existing_fact:
                new_fact = Fact(
                    question=fact,  # Storing question or identifying content
                    answer=fact,  # Storing fact itself
                    score=reliability_score,
                    timestamp=datetime.utcnow(),
                    extra_metadata=additional_info or {},
                )
                session.add(new_fact)
                session.commit()
                print(f"✅ Saved fact to knowledge base: {fact}")
            else:
                print(
                    "🚫 Fact already exists in knowledge base. Skipping save.")
    ...
    def search_facts(self, query: str) -> Optional[FactEntry]:
        session = get_db_session()
        try:
            facts = session.query(Fact).filter(
                Fact.question.ilike(f"%{query}%")).all()
            if facts:
                results = [
                    f"📖 {fact.question} -> {fact.answer} (Score: {fact.score})"
                    for fact in facts
                ]
                return results
            logger.info(f"🔍 No facts found matching '{query}'")
        except SQLAlchemyError as e:
...
def save_fact(
    ...
    *,
    ...
    """
    Save a fact with its certainty score (0-1).
    Args:
        fact: The factual information to save
        score: Certainty score 0-1 (0: uncertain, 1: absolutely certain)
    Returns:
    ...
    try:
        memory_store.save_fact(fact=fact,
                               reliability_score=score,
                               additional_info={
                                   "source": "tool",
                                   "user_id": user_id
                               })
        return f"✅ Saved fact (certainty: {score}★): {fact}"
    except Exception as e:
...
def search_facts_tool(query: str) -> str:
    """
    Search through verified facts.
    Args:
        query: The question to search for an answer.
    Returns:
        The answer from the local knowledge base if found.
    """
    session = get_db_session()
    try:
        facts = session.query(Fact).filter(
        ...
        if facts:
            ...
            logger.info(f"🔍 No verified facts found for: {query}")
    except SQLAlchemyError as e:
        logger.error(f"Error searching facts: {e}")
    finally:
        session.close()

    return f"🔍 No verified facts found for: {query}"
...
3. Save new important informations about user using the save_memory tool. 
4. Save new important facts or news or what you find in RAG and retrieved data using the fact_tool.(NOT with save_memory : that is for USER INFORMATIONS) - VERY IMPORTANT TO VERIFY USER CLAIM OR FACT THAT YOU DONT KNOW OR YOU HAVE DOUBT USE search tool. (travily) AND IF ITS CORRECT, USE IT.
5. USE RAG OPTIMIZELY ONLY WHEN YOU CANNOT ANSWER ABSOLUTE TO THE QUESTION. NOT ALWAYS. FOR YOUR OPTIMIZATION, LATANCY, INTELIGENCE AND SPEED.
...
def create_agent():
    ...
    tools = [search, save_memory, search_user_info, save_fact]
...
def process_response(chunk: Dict) -> str:
    responses = []
    try:
...
class ConversationManager:
    ...
    def start_conversation(self, user_id: str = None, thread_id: str = None):
        ...
        user_id = user_id or input(
            "Enter user ID (or press Enter for default 'user_1'): ").strip(
            ) or "user_1"
    ...
    # In the ConversationManager class
    ...
    def process_message(self, user_input: str) -> str:
        try:
            ...
            error_message = f"❌ Error processing message: {str(e)}"
            logger.error(error_message)  # Ensure logging captures this
            return error_message
    ...
    def end_conversation(self):
        ...
        """
        print("\n💾 Conversation ended. All messages are already saved.")
        print("👋 Thanks for chatting! Your conversation has been saved.")
...
/facts     - Search verified facts (follow with question)
        ...
        while True:
            try:
                ...
                if user_input.lower() == "/quit":
                    ...
                    continue
                elif user_input.lower().startswith("/facts "):
                    query = user_input[7:].strip()
                    if not query:
                        print("❗ Please provide a question after /facts.")
                        continue
                    print("\n" + search_facts_tool(query))
                    continue
